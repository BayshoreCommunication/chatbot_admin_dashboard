...
// Leads (from backend /lead routes)
...
export class AdminAPI {
  ...
  static async getSubscriptionDistribution(): Promise<
  ...
  static async getOrganizationWithApiKey(
  ...
  static async getOrganizationUsageAdmin(
    organizationId: string
  ): Promise<OrganizationUsageAdmin> {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const dailyConversations =
        Math.floor((usage.total_conversations || 0) / 30) +
        Math.floor(Math.random() * 5)
      const dailyUsers =
        Math.floor((usage.total_users || 0) / 30) +
  ...
  static async getConversationsBySession(
    sessionId: string
  ...
  // Leads - admin protected with static ADMIN_API_KEY header
  static async getLeads(): Promise<LeadItem[]> {
    // Source admin api key: localStorage override -> env -> default fallback
    const fromStorage =
      localStorage.getItem('adminApiKey') ||
      localStorage.getItem('ADMIN_API_KEY')
    const fromEnv = (import.meta as any).env?.VITE_ADMIN_API_KEY as
      | string
      | undefined
    const adminApiKey = fromStorage || fromEnv || 'admin-secret-key'
